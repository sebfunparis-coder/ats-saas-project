...
import { useAPIMissions, useAPICandidates, useAPIApplications, useAPIEvaluations, useAPIUsers, useAPIClients, useAPIEvents, useAPITasks, useAPITeam } from '../hooks/useAPIData';
import { useAuth } from './AuthContext';
import { useNotifications } from './NotificationsContext';
import { EQUIPIER_ROLES, OWNER_ROLES } from '../hooks/usePlanAccess';
import { generateAndDownloadDeletionProof } from '../utils/candidateDeletionProof';
import { deleteShareLinksForCandidate } from '../utils/shareLink';
import { deleteTrackingLinksForApplications } from '../utils/trackingLink';
import { isUUID } from '../utils/isUUID';
import { shouldAutoCloseMission } from '../utils/missionAutoClose';
...
// T-413 : 'archived' était compté dans les stats (CandidatesPage.jsx) et avait
// déjà son badge/couleur dédiés (CandidateCard/CandidateDetail, constants.js
// CANDIDATE_STATUS) mais n'était pas une option proposée ici — un recruteur ne
// pouvait l'atteindre qu'en tapant "archived" à la main dans le CreatableSelect.
const INITIAL_CANDIDATE_STATUSES = ['active', 'passive', 'hired', 'archived'];
...
export function DataProvider({ children }) {
  ...
  const {
    ...
    updateApplication: apiUpdateApplication,
  ...
  const {
    ...
    createEvaluation: apiCreateEvaluation,
    updateEvaluation: apiUpdateEvaluation,
    deleteEvaluation: apiDeleteEvaluation,
  } = useAPIEvaluations();
  ...
  const {
    ...
    updateTeamMember: apiUpdateTeamMember,
  ...
  // T-355 : un Équipier ne doit JAMAIS voir les données de démo fictives en repli.
  // Pour un Owner/Manager, un tableau vide signifie généralement "company toute
  // neuve, jamais utilisée" (le repli sur des données de démo est un choix produit
  // assumé). Pour un Équipier, un tableau vide signifie "aucune mission/candidat/
  // candidature qui lui est assigné(e) en ce moment" — un état réel et légitime
  // (déjà appliqué côté RLS par les policies *_equipier_assigned_only/allowed_only,
  // migrations 016/021). Lui montrer les données fictives d'une autre "company"
  // simulerait un accès à du travail qui ne lui appartient pas.
  const isEquipier = EQUIPIER_ROLES.includes(authUser?.role) && !OWNER_ROLES.includes(authUser?.role);

  // 🔥 États pour les entités BACKEND (fallback sur données initiales si API vide)
  //
  // T-386 : ce fallback s'appliquait dès que le tableau API était vide, SANS
  // jamais distinguer "encore en cours de chargement" de "réellement vide en
  // base" — une company fraîchement créée voyait donc les données de démo
  // fictives en PERMANENCE (pas juste le temps du chargement), et toute
  // tentative de modifier/supprimer l'un de ces éléments fictifs (id non-UUID
  // du type "1", "2"...) plantait avec une erreur Postgres 22P02 illisible
  // pour l'utilisateur. Ajout du garde `xLoading` : tant que la requête n'a
  // pas fini, on ne montre PAS encore le fallback fictif (évite le flash de
  // fausses données avant que le vrai résultat, potentiellement non-vide,
  // n'arrive) — le comportement de démo pour une company réellement vide
  // reste inchangé une fois le chargement terminé (produit intentionnel sur
  // ce projet, cf. T-355/T-356).
  const missions = missionsLoading ? (apiMissions || []) : ((apiMissions && apiMissions.length > 0) ? apiMissions : (isEquipier ? [] : INITIAL_MISSIONS));
  const setMissions = () => {};
  const candidates = candidatesLoading ? (apiCandidates || []) : ((apiCandidates && apiCandidates.length > 0) ? apiCandidates : (isEquipier ? [] : INITIAL_CANDIDATES));
  const setCandidates = () => {};
  // Applications — Supabase (T-245 : migré depuis un state local non persisté,
  // nécessaire pour que les candidatures soumises depuis le portail carrières
  // public apparaissent réellement dans le pipeline du recruteur)
  // Fallback local modifiable pour les données mock (quand Supabase est vide)
  const [mockApplications, setMockApplications] = useState(INITIAL_APPLICATIONS);
  const applications = applicationsLoading ? (apiApplications || []) : ((apiApplications && apiApplications.length > 0) ? apiApplications : (isEquipier ? [] : mockApplications));

  // Clients, events, tasks — Supabase si disponible, sinon localStorage/INITIAL
  const clients = clientsLoading ? (apiClients || []) : ((apiClients && apiClients.length > 0) ? apiClients : (saved?.clients || INITIAL_CLIENTS));
  const setClients = () => {};
  const events = (apiEvents && apiEvents.length > 0) ? apiEvents : (saved?.events || INITIAL_EVENTS);
  const setEvents = () => {};
  const tasks = (apiTasks && apiTasks.length > 0) ? apiTasks : (saved?.tasks || INITIAL_TASKS);
  const setTasks = () => {};
  ...
  const team = teamLoading ? (apiTeam || []) : ((apiTeam && apiTeam.length > 0) ? apiTeam : (saved?.team || INITIAL_TEAM));
  const setTeam = () => {};
  ...
  // Évaluations — Supabase (T-247 : nécessaire pour la consolidation multi-recruteurs,
  // impossible avec un state local par navigateur)
  const evaluations = evaluationsLoading ? (apiEvaluations || []) : ((apiEvaluations && apiEvaluations.length > 0) ? apiEvaluations : INITIAL_EVALUATIONS);
  ...
  }, [history, tags, companies, team, marketingCampaigns, promoCodes, supportTickets, candidateStatuses, candidateSources, candidateSectors]);
  ...
  // T-244 — Clôture auto des missions dont la date de clôture prévue est dépassée.
  // Pas de cron côté serveur (pas de backend déployé) : vérification passive à chaque
  // chargement/rafraîchissement des missions depuis Supabase.
  useEffect(() => {
    if (!apiMissions || apiMissions.length === 0) return;
    const today = new Date().toISOString().split('T')[0];
    apiMissions.forEach(m => {
      const closeDecision = shouldAutoCloseMission(m, { today });
      if (closeDecision?.reason === 'expired_date') {
        updateMission(m.id || m._id, { status: 'closed' })
          .then(() => notifyInfo('Mission clôturée automatiquement', closeDecision.message))
          .catch(err => console.error('Erreur clôture auto mission (date dépassée):', err));
      }
    });
  }, [apiMissions]);
  ...
  // T-225 — Droit à l'oubli : suppression candidat en cascade + preuve de suppression.
  // Le CV et les notes sont des champs embarqués sur la ligne candidat (pas de stockage
  // séparé type S3 — voir note T-218) : supprimés automatiquement avec apiDeleteCandidate.
  const deleteCandidate = async (id) => {
    try {
      assertRealId(id, 'Ce candidat');
      const candidate = candidates.find(c => c.id === id);

      // Candidatures liées — T-329 : depuis la migration T-245 vers Supabase,
      // `applications` vient presque toujours d'`apiApplications` (pas de
      // `mockApplications`), donc un simple `setMockApplications` ne supprimait
      // jamais réellement les candidatures liées en base — elles restaient
      // orphelines après une suppression RGPD. On appelle désormais réellement
      // la suppression (Supabase pour un vrai UUID, retrait du state mock sinon).
      const relatedApplications = applications.filter(
        a => a.candidateId === id || (candidate && a.candidateName === candidate.name)
      );
      if (relatedApplications.length > 0) {
        const relatedIds = new Set(relatedApplications.map(a => a.id));
        await Promise.all(relatedApplications.map(a =>
          isUUID(a.id) ? apiDeleteApplication(a.id) : Promise.resolve()
        ));
        setMockApplications(prev => prev.filter(a => !relatedIds.has(a.id)));
      }

      // T-382 : `share_links`/`tracking_links` (T-249/T-251) n'ont pas de FK
      // ON DELETE CASCADE vers candidates/applications — purgées explicitement
      // ici pour que le droit à l'oubli couvre aussi ces traces résiduelles
      // (fiche candidat + avis manager nominatif pour l'une, lien de suivi
      // candidat pour l'autre).
      let deletedShareLinksCount = 0;
      let deletedTrackingLinksCount = 0;
      if (isUUID(id)) {
        deletedShareLinksCount = await deleteShareLinksForCandidate(id);
      }
      const relatedApplicationUUIDs = relatedApplications.map(a => a.id).filter(isUUID);
      if (relatedApplicationUUIDs.length > 0) {
        deletedTrackingLinksCount = await deleteTrackingLinksForApplications(relatedApplicationUUIDs);
      }

      // Entrées d'historique/audit liées à ce candidat (création, modifications…)
      const relatedHistoryEntries = history.filter(
        h => h.relatedTo?.type === 'candidate' && h.relatedTo?.id === id
      );
      if (relatedHistoryEntries.length > 0) {
        setHistory(prev => prev.filter(h => !(h.relatedTo?.type === 'candidate' && h.relatedTo?.id === id)));
      }

      // Références et consentement RGPD stockés séparément en localStorage
      try { localStorage.removeItem(`ats_refs_${id}`); } catch {}
      try {
        const RGPD_KEY = 'ats_rgpd_consents';
        const consents = JSON.parse(localStorage.getItem(RGPD_KEY) || '{}');
        if (consents[id]) {
          delete consents[id];
          localStorage.setItem(RGPD_KEY, JSON.stringify(consents));
        }
      } catch {}
      ...
      if (candidate) {
        generateAndDownloadDeletionProof({
          candidate,
          deletedApplicationsCount: relatedApplications.length,
          deletedHistoryEntriesCount: relatedHistoryEntries.length,
        ...
        // 🔥 Auto-track — trace la suppression elle-même (ajoutée après la purge ci-dessus)
        track(
          'Candidat supprimé',
          { type: 'candidate', id, name: candidate.name },
          `Suppression de ${candidate.name} (${relatedApplications.length} candidature(s) liée(s) supprimée(s))`,
          '🗑️'
        );
      }
    } catch (error) {
  ...
  // Fonctions CRUD pour Clients (Supabase)
  // T-332 : ces fonctions retournaient auparavant un objet factice "succès" en
  // cas d'échec Supabase (state local non persisté depuis la migration vers
  // l'API) — l'utilisateur voyait un toast de succès puis le client disparaissait
  // au rechargement, sans jamais voir d'erreur. On propage désormais l'erreur,
  // comme le font déjà addMission/addCandidate.
  const addClient = async (client) => {
    try {
      const newClient = await apiCreateClient(client);
      track('Client ajouté', { type: 'client', id: newClient.id, name: newClient.name }, `${newClient.name} - ${newClient.industry}`, '🏢');
      return newClient;
    } catch (error) {
  ...
  const updateClient = async (id, updates) => {
    try {
      assertRealId(id, 'Ce client');
      const updated = await apiUpdateClient(id, updates);
      track('Client modifié', { type: 'client', id, name: updated.name }, `Mise à jour de ${updated.name}`, '✏️');
      return updated;
    } catch (error) {
  ...
  const deleteClient = async (id) => {
    try {
      ...
      track('Client supprimé', { type: 'client', id, name: '' }, 'Suppression', '🗑️');
    } catch (err) {
      throw err;
    }
  };

  // 🔥 Fonctions CRUD pour Applications (Supabase — T-245)
  const addApplication = async (application) => {
    const newApplication = await apiCreateApplication(application);
    ...
    // T-244 — Clôture auto si le seuil de candidatures de la mission est atteint
    const mission = missions.find(m => String(m.id || m._id) === String(newApplication.missionId));
    if (mission) {
      const applicationsCount = nextApplications.filter(
        a => String(a.missionId) === String(mission.id || mission._id)
      ).length;
      const closeDecision = shouldAutoCloseMission(mission, { applicationsCount });
      if (closeDecision?.reason === 'max_applications') {
        updateMission(mission.id || mission._id, { status: 'closed' })
          .then(() => notifyInfo('Mission clôturée automatiquement', closeDecision.message))
          .catch(err => console.error('Erreur clôture auto mission (seuil candidatures):', err));
      }
    }
  ...
  // T-389 (point 3) : `isUUID` était redéfini localement ici avec exactement
  // la même regex que `core/utils/isUUID.js` (déjà utilisé par shareLink.js/
  // trackingLink.js) — importé désormais depuis la source unique partagée.

  // T-386 : les données de démo (INITIAL_MISSIONS/CANDIDATES/CLIENTS/TEAM/
  // EVALUATIONS, affichées en repli pour une company réellement vide — cf.
  // plus haut) ont des ids non-UUID ("1", "2"...). Tenter de les modifier/
  // supprimer envoyait auparavant l'id tel quel à Supabase, qui répondait par
  // une erreur Postgres 22P02 illisible pour l'utilisateur. Ce garde
  // transforme ça en message clair, avant même l'appel réseau.
  const assertRealId = (id, label) => {
    if (!isUUID(id)) {
      throw new Error(`${label} affiché·e est une donnée de démonstration — elle n'existe pas réellement et ne peut pas être modifiée ou supprimée.`);
    }
  };
  ...
  const updateApplication = async (id, updates) => {
    ...
    // T-370 : renseigne `hiredAt` une seule fois, au moment RÉEL de la
    // transition vers 'hired' — seule source fiable pour le calcul du délai
    // de recrutement (DashboardPage.jsx, AnalyticsPage.jsx), qui utilisaient
    // auparavant 3 calculs différents, tous faux ou vides (temps jusqu'à
    // aujourd'hui au lieu de jusqu'à l'embauche, ou colonne inexistante).
    const finalUpdates = (updates.status === 'hired' && application?.status !== 'hired' && !application?.hiredAt)
      ? { ...updates, hiredAt: new Date().toISOString() }
      : updates;

    let updated;
    if (isUUID(id)) {
      // Vrai UUID Supabase → appel API. Repli si la migration 029 (colonne
      // hiredAt) n'a pas encore été exécutée — pour ne jamais casser le
      // passage d'une candidature au statut "Recruté" en attendant.
      try {
        updated = await apiUpdateApplication(id, finalUpdates);
      } catch (err) {
        if (err?.code === 'PGRST204' && finalUpdates.hiredAt) {
          updated = await apiUpdateApplication(id, updates);
        } else {
          throw err;
        }
      }
    } else {
      // ID mock (entier) → mise à jour optimiste du state local uniquement
      updated = { ...application, ...finalUpdates };
      setMockApplications(prev => prev.map(a => a.id === id ? updated : a));
    }

    const nextApplications = applications.map(a => a.id === id ? updated : a);
    ...
    // T-243 — Offre multi-postes : clôture auto quand tous les postes sont pourvus
    if (application && updates.status === 'hired') {
      const mission = missions.find(m => String(m.id || m._id) === String(application.missionId));
      if (mission) {
        const hiredCount = nextApplications.filter(
          a => String(a.missionId) === String(mission.id || mission._id) && a.status === 'hired'
        ).length;
        const closeDecision = shouldAutoCloseMission(mission, { hiredCount });
        if (closeDecision?.reason === 'positions_filled') {
          updateMission(mission.id || mission._id, { status: 'closed' })
            .then(() => notifyInfo('Mission clôturée automatiquement', closeDecision.message))
            .catch(err => console.error('Erreur clôture auto mission:', err));
        }
      }
    }
  ...
  // T-332 : mêmes fallbacks "succès simulé" que addClient/updateClient corrigés
  // ci-dessus — l'erreur est désormais propagée plutôt que masquée.
  const addEvent = async (event) => {
    try {
      const newEvent = await apiCreateEvent(event);
      track('Événement créé', { type: 'event', id: newEvent.id, name: newEvent.title }, `${newEvent.title} - ${newEvent.date}`, '📅');
      return newEvent;
    } catch (error) {
  ...
  const updateEvent = async (id, updates) => {
    try {
      ...
      track('Événement modifié', { type: 'event', id, name: updated.title }, `Mise à jour de ${updated.title}`, '✏️');
      return updated;
    } catch (error) {
  ...
  const deleteEvent = async (id) => {
    try {
      ...
      track('Événement supprimé', { type: 'event', id, name: '' }, 'Suppression', '🗑️');
    } catch (err) {
      throw err;
    }
  };
  ...
  // T-389 (point 1) : `setHistory([newItem, ...history])` capturait `history`
  // via la closure au moment du rendu — deux appels rapprochés (avant que
  // React ne re-render entre les deux) écrasaient l'un des deux ajouts.
  // `track()` (juste au-dessus) utilise déjà le bon pattern (`prev => ...`) ;
  // appliqué ici et sur les fonctions `add*` suivantes pour la même raison.
  ...
  // Fonctions CRUD pour Team (Supabase ou localStorage fallback)
  const addTeamMember = async (member) => {
    try {
      const newMember = await apiAddTeamMember(member);
      track('Membre ajouté', { type: 'team', id: newMember.id, name: newMember.name }, `${newMember.name} - ${newMember.role}`, '👥');
      return newMember;
    } catch {
      const newMember = { id: Date.now(), name: `${member.firstName || ''} ${member.lastName || ''}`.trim(), ...member };
      track('Membre ajouté', { type: 'team', id: newMember.id, name: newMember.name }, `${newMember.name}`, '👥');
      return newMember;
    }
  };

  const updateTeamMember = async (id, updates) => {
    // T-386 : le catch ci-dessous avale silencieusement toute erreur (bug
    // préexistant distinct, hors scope ici) — le garde doit donc être placé
    // AVANT le try, sinon une tentative de modifier un membre fictif (données
    // de démo) serait catchée et affichée comme un succès.
    assertRealId(id, 'Ce membre');
    try {
      const updated = await apiUpdateTeamMember(id, updates);
      track('Membre modifié', { type: 'team', id, name: updated.name }, `Mise à jour de ${updated.name}`, '✏️');
      return updated;
    } catch {
      track('Membre modifié', { type: 'team', id, name: '' }, 'Mise à jour', '✏️');
    }
  };
  ...
  const deleteTeamMember = async (id) => {
    try {
      ...
      track('Membre supprimé', { type: 'team', id, name: '' }, 'Suppression', '🗑️');
    } catch (err) {
      throw err;
    }
  };
  ...
  // CRUD Evaluations (Supabase — T-247)
  const addEvaluation = async (evaluation) => {
    const newEvaluation = await apiCreateEvaluation(evaluation);
    track(
      'Évaluation créée',
  ...
  const updateEvaluation = async (id, updates) => { assertRealId(id, 'Cette évaluation'); return apiUpdateEvaluation(id, updates); };

  const deleteEvaluation = async (id) => { assertRealId(id, 'Cette évaluation'); return apiDeleteEvaluation(id); };

  // CRUD Tasks (Supabase ou localStorage fallback)
  const addTask = async (task) => {
    try {
      const newTask = await apiCreateTask(task);
      track('Tâche créée', task.relatedTo || { type: 'task', id: newTask.id, name: newTask.title }, `${newTask.title}`, '✅');
      return newTask;
    } catch {
  ...
  const updateTask = async (id, updates) => {
    try {
      ...
      // silently ignore if Supabase not available
    }
  };
  ...
  const value = {
    ...
    setClients,
    setApplications: setMockApplications, // mock fallback
    setEvents,
    setHistory,
    setTags,
    setCompanies,
