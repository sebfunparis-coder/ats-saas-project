    ...
    <div style={{ display: 'flex', gap: '4px', alignItems: 'center' }}>
      {[1, 2, 3, 4, 5].map((star) => (
        <button
          ...
          aria-label={`${star} étoile${star > 1 ? 's' : ''} — ${STAR_LABELS[star]}`}
        >
          ★
        </button>
      ))}
 ...
 */
export function EvaluationModal({ application, isOpen, onClose, existingEvaluation, missionCriteria }) {
  const { addEvaluation, updateEvaluation, evaluationCriteria } = useData();
  const { user } = useAuth();

  const criteria = missionCriteria || evaluationCriteria || CRITERIA_DEFAULT;
  ...
  const evaluatorName = `${user?.firstName || ''} ${user?.lastName || ''}`.trim() || user?.email || 'Recruteur';
  ...
  const handleSubmit = async () => {
    ...
    if (existingEvaluation) {
      await updateEvaluation(existingEvaluation.id, evalData);
    } else {
    ...
    <Modal isOpen={isOpen} onClose={onClose} size="lg">
      ...
      <Modal.Body>
        ...
        <div style={{ marginBottom: '20px' }}>
          ...
          <div style={{ padding: '10px 14px', background: '#F9FAFB', border: '1.5px solid #E5E7EB', borderRadius: '10px', fontSize: '14px', color: '#374151', fontWeight: '600' }}>
            {evaluatorName}
          </div>
        </div>
