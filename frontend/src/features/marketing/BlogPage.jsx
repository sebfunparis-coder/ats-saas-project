...
import { fetchBlogPosts, contentfulEnabled } from '@/services/contentfulCms';
import styles from './BlogPage.module.css';

/**
 * Page Blog - Articles sur le recrutement, l'IA, et les RH (T-300 : CMS headless)
 * Charge depuis Contentful si VITE_CONTENTFUL_SPACE_ID + VITE_CONTENTFUL_ACCESS_TOKEN
 * sont configurés — sinon utilise les 12 articles hardcodés (fallback).
 */
export function BlogPage() {
  ...
  useEffect(() => {
    if (!contentfulEnabled()) return;
    fetchBlogPosts({ limit: 50 }).then(articles => {
      if (articles && articles.length > 0) setCmsArticles(articles);
    }).catch(() => {});
  }, []);
  ...
  // T-300 — Utilise les articles Contentful si disponibles, sinon les articles hardcodés
