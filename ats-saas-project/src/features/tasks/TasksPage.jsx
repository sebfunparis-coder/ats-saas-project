    ...
    >
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: '16px', marginBottom: '28px' }}>
        {[
          ...
          { label: '⏳ À faire',     value: stats.pending,     grad: '#6B7280,#9CA3AF' },
          { label: '🔵 En cours',    value: stats.in_progress, grad: '#3B82F6,#60A5FA' },
          { label: '✅ Terminées',   value: stats.done,        grad: '#10B981,#34D399' },
          { label: '🚨 En retard',   value: stats.overdue,     grad: '#EF4444,#F87171' },
        ].map((s) => (
          <div key={s.label} style={{
            ...
            <div style={{
              fontSize: '28px', fontWeight: '900', marginBottom: '2px',
              background: `linear-gradient(135deg, ${s.grad})`,
              WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent',
            }}>{s.value}</div>
            <div style={{ fontSize: '11px', color: '#6B7280', fontWeight: '600', textTransform: 'uppercase' }}>{s.label}</div>
          </div>
        ))}
      </div>
