 ...
 */
export function CompanyDetailModal({ company, isOpen, onClose, onSave, onDelete }) {
  ...
  const handleSendEmail = () => {
    alert(`📧 Email envoyé à ${company.name}\n\nDe: SuperAdmin ATS Ultimate\nÀ: ${company.name}\nObjet: Message depuis le SuperAdmin\n\n✅ Email envoyé avec succès !`);
  };

  const handleLoginAs = () => {
    alert(`🔐 Connexion en tant que ${company.name}\n\n⚠️ Vous allez être connecté avec les droits de cette entreprise.\nToutes vos actions seront visibles dans les logs.\n\n🔒 Cette fonctionnalité nécessite une authentification 2FA.`);
  };

  const handleUpgrade = () => {
    const newPlan = company.plan === 'Starter' ? 'Professional' : 'Enterprise';
    alert(`⬆️ Upgrade ${company.name}\n\nPlan actuel: ${company.plan}\nNouveau plan: ${newPlan}\n\nMRR: ${company.plan === 'Starter' ? '€99 → €299' : '€299 → €6,500'}\n\n✅ Upgrade programmé pour la prochaine facturation !`);
  };

  const handleSuspend = () => {
    if (window.confirm(`⚠️ Suspendre le compte ${company.name} ?\n\nCette action bloquera l'accès à la plateforme.\nLes données seront conservées.`)) {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} size="xl">
      ...
      <Modal.Body>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '16px', marginBottom: '32px', padding: '24px', background: '#F9FAFB', borderRadius: '12px' }}>
          {[
        ...
        <div style={{ marginBottom: '24px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '12px' }}>
            ...
            <button
              ...
              style={{ padding: '12px', background: '#EF4444', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer', fontWeight: '700', fontSize: '14px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}>
              🔒 Suspendre Compte
            </button>
          </div>
        </div>
